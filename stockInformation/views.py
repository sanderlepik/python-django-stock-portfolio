    ...
    if 'new_stock' in request.POST:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if Stocks.objects.filter(symbol=new_stock.upper()).count() < 1:
                    try:
                        ...
                        stock_to_db = Stocks(symbol=new_stock.upper(), name=new_stock_name, price=new_stock_price, change=new_stock_change)
                        stock_to_db.save()
        ...
        if request.method == 'POST':
            ...
            try:
                Stocks.objects.filter(symbol=symbol).delete() # ei saa praegu exceptionit ilmselt
                stock_list = Stocks.objects.order_by('price')[:10]
